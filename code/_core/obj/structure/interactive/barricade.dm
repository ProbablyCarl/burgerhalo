...
/obj/structure/interactive/barricade/proc/can_pack_up(var/mob/caller)
...
/obj/structure/interactive/barricade/proc/pack_up(var/mob/caller)

	caller.visible_message(span("warning","\The [caller.name] packs up \the [src.name]."),span("notice","You pack up \the [src.name]."))

	var/obj/item/deployable/barricade/S = new(get_turf(src))
	INITIALIZE(S)
	FINALIZE(S)
...
/obj/structure/interactive/barricade/drop_on_object(var/mob/caller,var/atom/object,location,control,params)
